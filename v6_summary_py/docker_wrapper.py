...
class TTLDockerWrapper(TTLGraphDockerWrapper):
    @staticmethod
    def load_data(database_uri, input_data):
        graph = super().load_data(database_uri, input_data)

        ret = graph.query(input_data['query'])
        cols = [str(var) for var in ret.vars]
        data = {}
        for col in cols:
            data[col] = []
        ...
        for row in ret:
            for col in cols:
                data[col].append(row[col].toPython())
